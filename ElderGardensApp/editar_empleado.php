  ...
  $('#Enviar').click(function (){
            ...
            $.ajax({
                ...
                success: function(data){
                    if (data == '1'){
                        $("#msgbox").html("Empleado Editado Exitosamente.");
 ...
 if (isset($_GET['idpersona'])) 
{
 ...
 ?> 
  ...
  <form id="formulario-principal">
    ...
    <div class="form-group">
      <label for="cargo">Actualizar Cargo del empleado</label>
      <select class="form-control" id="cargo" name="cargo" required>
        <option value="<?php echo $cargo;?>"><?php echo nombrecargo($cargo);?></option>
        <?php
        $query = 'SELECT * FROM `cargo`';
        $result = mysql_query($query) or die('Consulta fallida: ' . mysql_error());
        while($fila = mysql_fetch_array($result, MYSQL_ASSOC))
        {
          if($fila['id_cargo'] != $cargo)
          {
