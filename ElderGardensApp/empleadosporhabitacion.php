...
<h3 class="well">Asignar Empleados a una Habitación</h3>	

		<div class="row">
			<div class="col-lg-6 col-md-4 well"><h4>Arrastre los Empleados a la habitación seleccionada</h4>
				<?php
				...
				while($fila = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					...
					<p  style="font-size: 20px">'.$nombrepersona.'  '.$apellido.'<br><img src="images/empleados.png" ></p>
			...
			<form id="formulario-principal">
				<div class="col-lg-6 col-md-8"><h4>Escoja la habitación en la cual trabajará el empleado</h4>
					<div id="SegundaDiv">			
			...
			<button id = "Asignar" type="submit" class="btn btn-primary btn-lg btn-block">Asignar Empleados</button>
			</form>
		</div> <!-- Fin class row -->
<?php
if(isset($_POST["numHab"]))
{
	...
	for ($i = 0; $i<count ($array_id_persona); $i++)
	{
		...
		{
			$id_empleado = $_POST["drag-".$array_id_persona[$i]];//guarda el valor de cada paciente que fue seleccionado en la puerta
			//inserta paciente por habitación
			date_default_timezone_set("America/Bogota");
			$query = "INSERT INTO `habitacion_persona` (`id_habitacion`, `id_persona`, `fecha_ini`) VALUES ('".$id_Hab."', '".$id_empleado."', '".date('d-m-Y')."');";
	        //$result = mysql_query($query);
	...
	if($exitos == true)
	{
		echo '<script type="text/javascript">
				...
				$("#msgbox").html("Empleados Asignados Exitosamente.");
	...
	}else
	{
		echo '<script type="text/javascript">
				...
				$("#msgbox").html(data);
                ocultar(0, "alert alert-danger");</script>';
	}	
}	
?>		
