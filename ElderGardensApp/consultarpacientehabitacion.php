...
if(isset($_POST["numero"]))
{
	...
	if($numresul>0)
	{
		...
		while($fila1 = mysql_fetch_array($resultado, MYSQL_ASSOC))
		{
			...
			$query = 'SELECT * FROM `persona` WHERE `id_persona`='.$idPerHab.' AND `tipo_persona` = 1';
...
$('#Consultar').click(function (){
	...
	if ($("#formulario-principal").validate().form() === true){ // valida campos del formulario
		cargando('<img src="images/ajax-loader.gif" /> Consultando pacientes');
...
<div id="SegundaDiv">
	<h3 class="well">Consultar Pacientes en una Habitación</h3>	
	<form id="formulario-principal">						
			<div class="form-group">
			    <label for="numero">Seleccione # de habitación</label>
			    <select class="form-control" id="numero" name="numero" required="required">
